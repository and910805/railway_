# app.py - 女朋友對話機器人（LINE webhook + SQLite + Weather + Photo tasks）
import os
import datetime
import requests
...
LINE_TARGET_USER_ID = os.getenv("LINE_TARGET_USER_ID")  # optional fallback push target (if couple not ready)
...
def _is_admin(user_id: str) -> bool:
    # 若你沒設 ADMIN_LINE_USER_IDS，就視為不限制
...
def help_text() -> str:
    return (
        ...
        "\n"
        "雙方身份/推播：\n"
        "  我是臭寶            - 把自己標記成女友（用於雙方推播）\n"
        "  我是臭晡晡          - 把自己標記成你（用於雙方推播）\n"
        "  加入推播            - 開啟接收雙方推播\n"
        "  退出推播            - 關閉接收雙方推播\n"
        "\n"
        "基本：\n"
        ...
        "  新增約會 <內容>\n"
        "\n"
        "提示：help / 說明 / 幫助 都可以。也兼容 /help 這種舊打法。"
    )
...
def push_to_couple(message: str, fallback_user_id: str | None = None):
    """
    推播給雙方（臭寶 + 臭晡晡）。若 DB 尚未標記好兩位，會 fallback：
    - 若 fallback_user_id 有值：至少推播給目前對話者
    - 若 LINE_TARGET_USER_ID 有值：再推播給 LINE_TARGET_USER_ID
    """
    ...
    # fallback 方案（避免你還沒把女友 userId 設成「臭寶」就測不到）
    fallback = []
    if fallback_user_id:
...
def handle_command(user_id: str, text: str) -> str:
    ...
    if cmd == "我是臭寶":
        ...
        return "✅ 已設定你是：臭寶（女友）。之後天氣提醒/任務會推播給你。"

    if cmd == "我是臭晡晡":
        set_role(user_id, "boyfriend")
        set_active(user_id, True)
        return "✅ 已設定你是：臭晡晡。之後天氣提醒/任務會推播給你。"
    ...
    # ====== 天氣 ======
    if cmd == "天氣":
        metrics = fetch_today_weather_metrics(
            lat=WEATHER_LAT, lon=WEATHER_LON, timezone=TIMEZONE
        )
    ...
    if cmd == "攝影任務":
        mode = "互拍"
        if arg.strip() in ("給臭寶", "給女友", "給臭寶寶"):
            mode = "給臭寶"
        elif arg.strip() in ("給臭晡晡", "給我", "給男友"):
            mode = "給臭晡晡"
...
def process_line_events(body):
    ...
    for ev in body.get("events", []):
        ...
        # 讓攝影任務交作業時有回饋（圖片/影片/貼圖等）
        if msg_type in ("image", "video"):
            print(f"[IN] {display_name}({user_id}): <{msg_type}> id={msg.get('id')}", flush=True)
            if reply_token:
...
if ENABLE_SCHEDULER:
    # 注意：gunicorn workers > 1 會啟動多份 scheduler，請保持 workers=1
    try:
        start_scheduler()
    except Exception as e:
